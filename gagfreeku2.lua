perbaiki autobuy menggunakan fireserver atau button Sheckles_Buy
...
-- Cari event Sheckles_Buy jika ada
local Sheckles_Buy = ReplicatedStorage:FindFirstChild("Sheckles_Buy") or ReplicatedStorage.GameEvents:FindFirstChild("Sheckles_Buy")
...
-- Fungsi yang ditingkatkan untuk membeli benih
local function buyCropSeeds(cropName)
    local args = {[1] = cropName}
    local success, errorMsg = pcall(function()
        -- Coba gunakan Sheckles_Buy jika tersedia
        if Sheckles_Buy and Sheckles_Buy:IsA("RemoteEvent") then
            Sheckles_Buy:FireServer(unpack(args))
            print("Used Sheckles_Buy for: " .. cropName)
        else
            -- Fallback ke BuySeedStock
            BuySeedStock:FireServer(unpack(args))
            print("Used BuySeedStock for: " .. cropName)
        end
    end)
    
    if not success then
        print("Error buying seeds for " .. cropName .. ":", errorMsg)
        return false
    end
    return true
end

-- Fungsi yang ditingkatkan untuk simulasi klik button
local function clickBuyButton(cropName)
    -- Cari button di GUI shop
    local success = false
    pcall(function()
        for _, plantFrame in pairs(SeedShopGUI:GetChildren()) do
            if plantFrame.Name == cropName and plantFrame:FindFirstChild("Main_Frame") then
                local buyButton = plantFrame.Main_Frame:FindFirstChild("Buy_Button")
                if buyButton and buyButton:IsA("TextButton") then
                    -- Simulasi klik button
                    buyButton:FireServer("MouseButton1Click")
                    success = true
                    print("Clicked buy button for: " .. cropName)
                    break
                end
            end
        end
...
function buyWantedCropSeeds()
    ...
    for _, fruitName in ipairs(wantedFruits) do
        ...
        if stock > 0 then
            -- Coba beberapa metode pembelian
            local purchaseMethods = {
                function() return buyCropSeeds(fruitName) end, -- Method 1: RemoteEvent
                function() return clickBuyButton(fruitName) end -- Method 2: GUI Button
            }
            
            for j = 1, stock do
                local bought = false
                
                -- Coba semua metode pembelian
                ...
                if not bought then
                    print("Failed to buy "..fruitName.." with all methods")
                end
...
-- Tambahkan section untuk debug pembelian
seedsTab:CreateSection("Debug Purchase Methods")
seedsTab:CreateButton({
    Name = "Test Buy Method 1 (RemoteEvent)",
    Callback = function()
        if #wantedFruits > 0 then
            local fruitName = wantedFruits[1]
            print("Testing RemoteEvent method for: " .. fruitName)
...
seedsTab:CreateButton({
    Name = "Test Buy Method 2 (GUI Button)",
    Callback = function()
        if #wantedFruits > 0 then
            local fruitName = wantedFruits[1]
            print("Testing GUI Button method for: " .. fruitName)
            clickBuyButton(fruitName)
        else
            print("No fruit selected")
        end
    end,
...
print("Sheckles_Buy found:", Sheckles_Buy ~= nil)
