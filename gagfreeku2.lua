tambahkan menu disamping seed adalah stock seed dan bisa dibeli langsung
...
local function autoPurchaseCheck()
    ...
    if currentStock >= threshold then
        print("Auto-purchasing " .. selectedSeed .. " (Stock: " .. currentStock .. ")")
        ...
        for i = 1, currentStock do
            local success = buyCropSeeds(selectedSeed)
            if success then
                print("Auto-purchased " .. selectedSeed .. " seed " .. i .. "/" .. currentStock)
            else
                print("Failed to auto-purchase " .. selectedSeed)
                break
            end
            wait(0.2)
        end
