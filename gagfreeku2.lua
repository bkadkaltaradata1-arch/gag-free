...
-- PERBAIKAN: Fungsi untuk mencari dan mengklik button sheckles_buy
local function findAndClickShecklesBuy(cropName)
    -- Cari plant frame berdasarkan nama
    local plantFrame = SeedShopGUI:FindFirstChild(cropName)
    if not plantFrame then
        print("Plant frame not found for: " .. cropName)
        return false
    end
    
    -- Masuk ke Main_Frame
    local mainFrame = plantFrame:FindFirstChild("Main_Frame")
    if not mainFrame then
        print("Main_Frame not found for: " .. cropName)
        return false
    end
    
    -- Cari button sheckles_buy
    local shecklesBuyButton = mainFrame:FindFirstChild("sheckles_buy")
    if not shecklesBuyButton then
        print("sheckles_buy button not found for: " .. cropName)
        return false
    end
    
    -- Simulasikan klik pada button
    local success, errorMsg = pcall(function()
        -- Coba fire event click jika ada
        if shecklesBuyButton:IsA("TextButton") or shecklesBuyButton:IsA("ImageButton") then
            shecklesBuyButton:FireEvent("MouseButton1Click")
        end
        
        -- Juga coba aktifkan event mouse click
        shecklesBuyButton:FireEvent("Activated")
    end)
    
    if success then
        print("Successfully clicked sheckles_buy for: " .. cropName)
        return true
    else
        print("Error clicking sheckles_buy: " .. tostring(errorMsg))
        return false
    end
end

local function buyCropSeeds(cropName)
    -- Coba metode pertama: menggunakan remote event
    ...
    if success then
        print("Bought " .. cropName .. " using remote event")
        return true
    end
    
    -- Jika metode pertama gagal, coba metode kedua: klik button sheckles_buy
    print("Remote event failed, trying sheckles_buy button...")
    return findAndClickShecklesBuy(cropName)
end
...
spawn(function() 
    while true do
        if shopTimer and shopTimer.Text then
            ...
            local isRefreshed = getCropsListAndStocks()
